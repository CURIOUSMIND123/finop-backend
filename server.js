...
// CORS Configuration - Allow one.com domain with credentials
const corsOptions = {
  origin: process.env.FRONTEND_URL || 'https://yourdomain.one.com',
  credentials: true,
...
async function initializeDatabase() {
  try {
    ...
    console.log('✅ Database tables already exist or ready:', error.message.substring(0, 50));
  }
}
...
async function getNifty50Price() {
  try {
    ...
    return 25142 + (Math.random() - 0.5) * 100;
  }
}
...
async function getSensexPrice() {
  try {
    ...
    return 82450 + (Math.random() - 0.5) * 200;
  }
}
...
function calculateGreeks(spot, strike, daysToExpiry, volatility, riskFreeRate = 6.5) {
  ...
  if (T <= 0 || sigma <= 0 || spot <= 0) {
    return { error: 'Invalid parameters: T, sigma, and spot must be positive' };
  }
...
app.get('/api/live-data', async (req, res) => {
  try {
    ...
    const niftyChange = (Math.random() - 0.5) * 50;
    const sensexChange = (Math.random() - 0.5) * 100;

    res.json({
      nifty: {
        price: parseFloat(nifty.toFixed(2)),
        change: parseFloat(niftyChange.toFixed(2)),
        changePercent: parseFloat(((niftyChange / nifty) * 100).toFixed(2))
      },
      sensex: {
        price: parseFloat(sensex.toFixed(2)),
        change: parseFloat(sensexChange.toFixed(2)),
        changePercent: parseFloat(((sensexChange / sensex) * 100).toFixed(2))
      },
...
(async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`CORS Origin: ${process.env.FRONTEND_URL || 'https://yourdomain.one.com'}`);
